...
def createBlogHandler(request) -> blog_service_pb2.CreateBlogResponse:
  ...
  if not newBlog.writerId:
    return blog_service_pb2.CreateBlogResponse(
        isSuccess=False, errorMsg="User must be authenticated", url=""
    )
  elif not newBlog.title:
    return blog_service_pb2.CreateBlogResponse(
        isSuccess=False, errorMsg="Title must be provided", url=""
    )
  elif not newBlog.content:
    return blog_service_pb2.CreateBlogResponse(
        isSuccess=False, errorMsg="Content must be provided", url=""
    )
  ...
  if blog:
    return blog_service_pb2.CreateBlogResponse(
        isSuccess=False,
        errorMsg="Blog by the same title already exist",
        url="",
    )
  ...
  return blog_service_pb2.CreateBlogResponse(
      isSuccess=True, errorMsg="", url=blog.url
  )
...
def getBlogDetailHandler(request) -> blog_service_pb2.GetBlogDetailResponse:
  ...
  with grpc.insecure_channel("user-service:50051") as channel:
    ...
    writer = stub.GetUserById(
        user_service_pb2.GetUserByIdRequest(id=blog.writerId))
...
def getBlogDetailErrorResponse(
    errorMsg: str,
) -> blog_service_pb2.GetBlogDetailResponse:
